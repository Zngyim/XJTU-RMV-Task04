...
class MVSCameraDisplayNode : public rclcpp::Node
{
public:
    MVSCameraDisplayNode() : Node("mvs_camera_display_node"), connected_(false)
    {
        RCLCPP_INFO(this->get_logger(), "MVS Camera Display Node 启动中...");

        // 创建 ROS2 发布器
        
        this->declare_parameter<double>("exposure_time", 15000.0);
        ...
        param_callback_handle_ = this->add_on_set_parameters_callback(
            std::bind(&MVSCameraDisplayNode::onParameterChange, this, std::placeholders::_1)
        );
    ...
    static void __stdcall ExceptionCallback(unsigned int nMsgType, void* pUser)
    {
        ...
        {
            auto* node = static_cast<MVSCameraDisplayNode*>(pUser);
    ...
    void grabAndShow()
    {
        ...
        if (nRet == MV_OK)
        {
            ...
            cv::imshow("MVS Camera Stream", img);
...
int main(int argc, char** argv)
{
    ...
    rclcpp::spin(std::make_shared<MVSCameraDisplayNode>());
    rclcpp::shutdown();
    return 0;
}
